...
def generate_samples():
    ...
    with open('driving_log.csv') as csvfile:
        ...
        for center_image,_,_,steering_angle,_,_, speed in csv.reader(csvfile):
            ...
            # counter +=1
            if float(steering_angle)**2 > 0.02 or float(steering_angle)**2 + .05 > random.random():
...
model = Sequential([
    ...
    Conv2D(16, strides=3, kernel_size=3, activation='elu', input_shape=(50, 160, 1), kernel_regularizer=regularizers.l2(.001)),
    Dropout(.2),
